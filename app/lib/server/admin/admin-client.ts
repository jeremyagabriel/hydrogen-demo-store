...
type AdminClientReturn = {
  ...
  /**
   * Returns an object that contains headers that are needed for each query to Admin API GraphQL endpoint. This method uses the private Server-to-Server token which reduces the chance of throttling but must not be exposed to clients. Server-side calls should prefer using this over `getPublicTokenHeaders()`.
   *
